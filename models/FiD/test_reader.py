        ...
        for i, batch in enumerate(dataloader):
            ...
            for k, o in enumerate(outputs):
                ...
                if 'answers' in example:
                    score = src.evaluation.f1s(ans, example['answers'], opt.use_pi_f1)
                    scores.append(score)
            ...
            if (i + 1) % opt.eval_print_freq == 0:
                ...
                if len(scores) == 0:
                    ...
                    log += f' | average = {np.mean(scores):.3f}'
                logger.warning(log)

    logger.warning(f'Process rank:{opt.global_rank}, total {total} | average = {np.mean(scores):.3f}')
    if opt.is_distributed:
        torch.distributed.barrier()
    score, total = src.util.weighted_average(np.mean(scores), total, opt)
...
if __name__ == "__main__":
    ...
    logger.info("Current device is %s", opt.device)
    logger.info("Start eval")
    ave_score, total = evaluate(model, eval_dataset, eval_dataloader, tokenizer, opt)

    if opt.use_pi_f1:
        logger.info(f'PI-F1 {100*ave_score:.2f}, Total number of example {total}')
    else:
        logger.info(f'RG-F1 {100*ave_score:.2f}, Total number of example {total}')
