...
def f1s(prediction, ground_truths, use_pi_f1):
    if use_pi_f1:
        def _get_pids(s):
            pids = s.split('answer: ')[0].replace('evidence:', '').split('|')
            return list(set([p.strip() for p in pids]))
        pred_passages = _get_pids(prediction)
        ground_truths = [_get_pids(g) for g in ground_truths]
        return eval_evidence_passages([pred_passages], [ground_truths])
    else:
